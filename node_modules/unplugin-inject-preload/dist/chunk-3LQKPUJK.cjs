...
function getAssetsForWebpackOrRspack(compilation) {
  ...
  if (isWebpack) {
    ...
    const assets2 = compilation.getAssets() || [];
    assetsInfo = new Map(
      assets2.map((asset) => [asset.name, asset.info])
    );
    outputs = Array.from(assets2.map((asset) => asset.name)).sort();
  }
  outputs.forEach((output) => {
  ...
  for (let i = 0; i < assets.length; i++) {
    ...
    for (let index = 0; index < options.files.length; index++) {
      ...
      if (basePath.indexOf("://") > 0)
        ...
        href = (basePath.startsWith("./") ? "./" : "") + _path2.default.posix.join(basePath, asset.output);
...
function htmlRspackPluginAdapter(args) {
  ...
  compiler.hooks.emit.tapAsync(name2, (compilation, callback) => {
    ...
    if (!pluginInstances)
      return callback();
    const filenames = pluginInstances.flatMap((item) => item && "_args" in item ? [item._args.filename || "index.html"] : []);
